...
from etl_core.api.cli import (
    app, LocalJobsClient, LocalExecutionClient, 
    HttpJobsClient, HttpExecutionClient, _pick_clients
)
...
class TestHttpJobsClient:
    ...
    def test_get_job_not_found(self, mock_requests):
        ...
        with pytest.raises(PersistNotFoundError, match="Job 'test-job-id-123' not found"):
...
class TestClientSelection:
    ...
    def test_pick_http_clients(self):
        ...
        jobs_client, exec_client = _pick_clients(remote=True, base_url="http://localhost:8000")
...
class TestCLICommands:
    ...
    def test_create_job_command_remote(self, runner, temp_config_file, mock_requests):
        ...
        result = runner.invoke(app, [
            "create-job", str(temp_config_file), 
            "--remote", "--base-url", "http://localhost:8000"
        ])
    ...
    def test_get_job_command_not_found(self, runner, mock_singletons):
        ...
        mock_jh, mock_eh = mock_singletons
        mock_jh.return_value.load_runtime_job.side_effect = PersistNotFoundError("Job not found")
    ...
    def test_update_job_command_local(self, runner, temp_config_file, mock_singletons):
        ...
        result = runner.invoke(app, ["update-job", "test-job-id-123", str(temp_config_file)])
        
        assert result.exit_code == 0
        assert "Updated job updated-job-123" in result.stdout
    
    def test_update_job_command_not_found(self, runner, temp_config_file, mock_singletons):
        """Test update_job command with non-existent job."""
        mock_jh, mock_eh = mock_singletons
        mock_jh.return_value.update.side_effect = PersistNotFoundError("Job not found")
        
        result = runner.invoke(app, ["update-job", "non-existent-id", str(temp_config_file)])
