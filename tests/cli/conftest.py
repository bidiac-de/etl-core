...
from tests.helpers import get_sample_job_config, create_temp_job_config, cleanup_temp_file
...
def mock_job_handler():
    ...
    mock.load_runtime_job.return_value = Mock(
        id="test-job-id-123",
        model_dump=lambda: get_sample_job_config()
    )
...
def mock_execution_handler():
    ...
    mock = Mock()
    mock.execute_job.return_value = Mock(
        id="exec-123",
        max_attempts=3
    )
    return mock
