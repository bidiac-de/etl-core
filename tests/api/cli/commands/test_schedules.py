...
class TestCreateCommand:
    ...
    def test_create_without_trigger_args(self, mock_command_class, mock_api_base_url):
        ...
        with patch("typer.echo") as mock_echo:  # noqa: F841
            create(
...
class TestListCommand:
    ...
    def test_list_without_api_url(self, mock_handler_singleton, mock_api_base_url):
        ...
        expected_calls = [
            (
                (
                    "local-schedule-1\tLocal Schedule "
                    "1\tlocal-job-1\tTEST\tdate\tpaused=False",
                ),
            ),
            (
                (
                    "local-schedule-2\tLocal Schedule "
                    "2\tlocal-job-2\tDEV\tinterval\tpaused=True",
                ),
            ),
        ]
        assert mock_echo.call_count == 2
...
class TestGetCommand:
    ...
    def test_get_with_api_url_not_found(self, mock_get, mock_api_base_url):
        ...
        mock_get.assert_called_once_with(
            "http://test-api.com/schedules/nonexistent-schedule"
        )

    @patch("etl_core.api.cli.commands.schedules.api_base_url")
    @patch("etl_core.api.cli.commands.schedules._schedule_handler_singleton")
    def test_get_without_api_url_success(
        self, mock_handler_singleton, mock_api_base_url
    ):
    ...
    @patch("etl_core.api.cli.commands.schedules.api_base_url")
    @patch("etl_core.api.cli.commands.schedules._schedule_handler_singleton")
    def test_get_without_api_url_not_found(
        self, mock_handler_singleton, mock_api_base_url
    ):
...
class TestUpdateCommand:
    ...
    def test_update_with_api_url_success(self, mock_put, mock_api_base_url):
        ...
        mock_put.assert_called_once_with(
            "http://test-api.com/schedules/schedule-123", json=expected_payload
        )
        mock_response.raise_for_status.assert_called_once()
        mock_echo.assert_called_once_with(
            {"id": "updated-schedule", "name": "Updated Name"}
        )
...
class TestDeleteCommand:
    ...
    def test_delete_with_api_url_success(self, mock_delete, mock_api_base_url):
        ...
        mock_delete.assert_called_once_with(
            "http://test-api.com/schedules/schedule-to-delete"
        )
        mock_response.raise_for_status.assert_called_once()
...
class TestPauseCommand:
    ...
    def test_pause_with_api_url_success(self, mock_post, mock_api_base_url):
        ...
        mock_post.assert_called_once_with(
            "http://test-api.com/schedules/schedule-to-pause/pause"
        )
        mock_response.raise_for_status.assert_called_once()
        mock_echo.assert_called_once_with("OK")
...
class TestResumeCommand:
    ...
    def test_resume_with_api_url_success(self, mock_post, mock_api_base_url):
        ...
        mock_post.assert_called_once_with(
            "http://test-api.com/schedules/schedule-to-resume/resume"
        )
        mock_response.raise_for_status.assert_called_once()
        mock_echo.assert_called_once_with("OK")
...
class TestRunNowCommand:
    ...
    def test_run_now_with_api_url_success(self, mock_post, mock_api_base_url):
        ...
        mock_post.assert_called_once_with(
            "http://test-api.com/schedules/schedule-to-run/run-now"
        )
        mock_response.raise_for_status.assert_called_once()
    ...
    @patch("asyncio.run")
    def test_run_now_without_api_url(
        self, mock_asyncio_run, mock_command_class, mock_api_base_url
    ):
