...
class TestRealDatabaseConnection:
    ...
    def test_can_connect_to_database(self, real_credentials):
        ...
        try:
            ...
            password = real_credentials.decrypted_password if real_credentials.decrypted_password else ""
            url = f"mysql+mysqlconnector://{real_credentials.user}:{password}@{real_credentials.host}:{real_credentials.port}/{real_credentials.database}"
    ...
    def test_can_read_from_database(self, real_context, test_table_name):
        ...
        try:
            ...
            # Build connection URL manually since build_url doesn't handle empty passwords
            password = credentials.decrypted_password if credentials.decrypted_password else ""
            url = f"mysql+mysqlconnector://{credentials.user}:{password}@{credentials.host}:{credentials.port}/{credentials.database}"
            ...
            with handler.lease() as connection:
                ...
                print("✅ Successfully tested MariaDBRead component with real credentials!")
    ...
    def test_can_write_to_database(self, real_context, test_table_name):
        ...
        try:
            ...
            pytest.fail(f"Failed to test database write: {e}")
    ...
    def test_connection_pool_settings(self, real_credentials):
        ...
        try:
            ...
            # Build connection URL manually since build_url doesn't handle empty passwords
            password = pool_credentials.decrypted_password if pool_credentials.decrypted_password else ""
            url = f"mysql+mysqlconnector://{pool_credentials.user}:{password}@{pool_credentials.host}:{pool_credentials.port}/{pool_credentials.database}"
    ...
    def test_connection_error_handling(self, real_context):
        ...
        try:
            ...
            invalid_url = "mysql+mysqlconnector://invalid_user:wrong_password@localhost:3306/etl_core"
            ...
            try:
                ...
                print("⚠️  WARNING: Invalid credentials were accepted! This indicates weak authentication.")
                print("⚠️  MariaDB accepted 'invalid_user:wrong_password' - security concern!")
            ...
            print(f"✅ Expected connection failure with invalid credentials: {e}")
    ...
    def test_database_schema_operations(self, real_context):
        ...
        try:
            ...
            pytest.fail(f"Failed to test schema operations: {e}")
        ...
        try:
            ...
            with handler.lease() as connection:
                ...
                if 'test_connection_table' in tables:
                    result = connection.execute(text("SELECT * FROM test_connection_table"))
                    rows = result.fetchall()
    ...
    def test_cleanup_test_data(self, real_context):
        ...
        try:
            ...
            with handler.lease() as connection:
                ...
                result = connection.execute(text("SHOW TABLES LIKE 'test_connection_table'"))
                ...
                if tables:
                    ...
                    print("🧹 Test table 'test_connection_table' was already cleaned up")
