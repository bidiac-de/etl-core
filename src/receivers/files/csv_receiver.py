...
from typing import Dict, Any, List, Generator, Union, AsyncIterator
import pandas as pd
...
from src.receivers.files.file_helper import resolve_file_path
...
class CSVReceiver(ReadFileReceiver, WriteFileReceiver):
    ...
    async def read_row(self, metrics: ComponentMetrics) -> AsyncIterator[Dict[str, Any]]:
        """Stream all CSV rows as dictionaries (async generator)."""
        rows = await asyncio.to_thread(lambda: list(read_csv_row(self.filepath)))
        for row in rows:
            yield row

    async def read_bulk(self, metrics: ComponentMetrics) -> pd.DataFrame:
        """Read the entire CSV into a Pandas DataFrame."""
        return await asyncio.to_thread(read_csv_bulk, self.filepath)

    async def read_bigdata(self, metrics: ComponentMetrics, blocksize: str = "16MB") -> dd.DataFrame:
        """Read large CSV files efficiently using Dask."""
        return await asyncio.to_thread(read_csv_bigdata, self.filepath, blocksize)

    async def write_row(self, metrics: ComponentMetrics, row: Dict[str, Any]):
        """Write a single row to the CSV file."""
        await asyncio.to_thread(write_csv_row, self.filepath, row)

    async def write_bulk(self, metrics: ComponentMetrics, data: List[Dict[str, Any]]):
        """Write multiple rows to the CSV file."""
        if data:
            await asyncio.to_thread(write_csv_bulk, self.filepath, data)
    ...
    async def write_bigdata(self, metrics: ComponentMetrics, data: dd.DataFrame):
        ...
        await asyncio.to_thread(write_csv_bigdata, self.filepath, data)